import unittest
from inventory.urlBuilder import *


class TestUrlBuilder(unittest.TestCase):

    def test_search_with_filter(self):
        url = Search.search_by("allergy").filter_by(FilterBy.VEGETARIAN).url()
    ...
    def test_category_two_categories(self):
        url = Category.ACHES_AND_PAINS.ACCESSORIES.url()
    ...
    def test_category_with_multiple_filter_and_sort(self):
        url = Category.ALLERGY_AND_HAYFEVER\
    ...
    def test_popularity_with_sort(self):
        url = Popular.show_all().sort_by(SortBy.POPULARITY).url()
